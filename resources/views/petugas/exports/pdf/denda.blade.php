    ...
    <style>
        * {
        ...
        .watermark {
            ...
            letter-spacing: 10px;
            border: 5px double rgba(220,38,38,0.08);
            padding: 25px 45px;
            border-radius: 15px;
            pointer-events: none;
        }
...
<body>
    ...
    <div class="content-wrapper">
        ...
        @php
            ...
            $startLabel = $start_date ? \Carbon\Carbon::parse($start_date)->format('d/m/Y') : 'Semua';
            $endLabel = $end_date ? \Carbon\Carbon::parse($end_date)->format('d/m/Y') : 'Semua';
        @endphp
        ...
        <div class="ringkasan">
            <table>
                <tr>
                    ...
                    <td>{{ $startLabel }} - {{ $endLabel }}</span></td>
                    <td width="50"></td>
                    <td width="120"><strong>Total Denda:</strong></td>
                    <td>Rp {{ number_format($totalDenda, 0, ',', '.') }}</span></td>
                </tr>
                <tr>
                    <td><strong>Jumlah Transaksi:</strong></td>
                    <td>{{ $totalTransaksi }} transaksi</span></td>
                    <td></td>
                    <td><strong>Rata-rata Denda:</strong></td>
                    <td>Rp {{ number_format($rataDenda, 0, ',', '.') }}</span></td>
                </tr>
                <tr>
                    <td><strong>Denda Tertinggi:</strong></td>
                    <td>Rp {{ number_format($maksDenda, 0, ',', '.') }}</span></td>
                    <td></span></td>
                    <td></span></td>
                </tr>
            </table>
        ...
        <table class="data">
            ...
            <tbody>
                ...
                @php
                    ...
                    $jatuhTempo = $peminjaman ? \Carbon\Carbon::parse($peminjaman->tgl_jatuh_tempo) : null;
                    $kembali = $peminjaman ? \Carbon\Carbon::parse($peminjaman->tanggal_pengembalian) : null;
                    $terlambat = ($kembali && $jatuhTempo && $kembali->gt($jatuhTempo)) ? $jatuhTempo->diffInDays($kembali) : 0;
                @endphp
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td class="text-center">{{ $peminjaman ? \Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->format('d/m/Y') : '-' }}</td>
                    <td>{{ $anggota->name ?? '-' }}<br><small>{{ $anggota->no_anggota ?? '-' }}</small></td>
                    <td>{{ $buku->judul ?? '-' }}<br><small>{{ $buku->pengarang ?? '-' }}</small></td>
                    <td class="text-center">{{ $kembali ? $kembali->format('d/m/Y') : '-' }}</td>
                    <td class="text-center">{{ $jatuhTempo ? $jatuhTempo->format('d/m/Y') : '-' }}</td>
                    <td class="text-center">{{ $terlambat }} hari</span></td>
                    <td class="text-right denda">Rp {{ number_format($item->jumlah_denda, 0, ',', '.') }}</span></td>
